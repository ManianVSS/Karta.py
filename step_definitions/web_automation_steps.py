...
from step_definitions._w3schools import HomePage, HTMLHomePage, HTMLIntroductionPage
...
def w3schools_learn_html_home_page_is_opened(context=None):
    logger.info("context is %s", str(context))
    learn_html_page = context.w3schools_app.home_page.open_learn_html_page()
    return isinstance(learn_html_page, HTMLHomePage)


@step_def("w3schools learn html home page is opened from html introduction page")
def w3schools_learn_html_home_page_is_opened_from_html_introduction_page(context=None):
    logger.info("context is %s", str(context))
    learn_html_page = context.w3schools_app.html_introduction_page.open_learn_html_page()
    return isinstance(learn_html_page, HTMLHomePage)
