...
def generate_next_composition(probability_map: dict, random: Random) -> list[object]:
    chosen_objects = []
...
def generate_next_mutex_composition(probability_map: dict, random: Random) -> object:
    if not probability_map or (len(probability_map) == 0):
        return None
