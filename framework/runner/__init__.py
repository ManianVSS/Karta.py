...
def karta_main(args=None):
    try:
        ...
        if parsed_args.tags:
            print("Tags to run {}".format(parsed_args.tags))
            feature_results = karta_runtime.run_tags(parsed_args.tags)
            print(str(feature_results))
        elif parsed_args.features:
            print("Features to run {}".format(parsed_args.features))
